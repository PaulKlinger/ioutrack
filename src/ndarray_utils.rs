...
use ndarray::prelude::*;
use ndarray::Zip;
use crate::num_utils::{BboxNum, partial_min, partial_max};
...
pub fn maximum<T: BboxNum, D1: Dimension, D2: Dimension>(a: ArrayView<T, D1>, b: ArrayView<T, D2>) -> anyhow::Result<Array<T, IxDyn>>{
    let mut res = Array::zeros(get_broadcast_shape(a.shape(), b.shape())?);
    Zip::from(&mut res).and_broadcast(&a).and_broadcast(&b).for_each(|r, &x, &y| {*r = partial_max(x, y)});
    Ok(res)
}

/// Elementwise minimum of two arrays, with numpy-style broadcasting
pub fn minimum<T: BboxNum, D1: Dimension, D2: Dimension>(a: ArrayView<T, D1>, b: ArrayView<T, D2>) -> anyhow::Result<Array<T, IxDyn>>{
    let mut res = Array::zeros(get_broadcast_shape(a.shape(), b.shape())?);
    Zip::from(&mut res).and_broadcast(&a).and_broadcast(&b).for_each(|r, &x, &y| {*r = partial_min(x, y)});
    Ok(res)
}
...
mod tests {
    ...
    fn test_maximum() {
        ...
        let b = array![[3], [-1]];
        let max = maximum(a.view(), b.view()).unwrap().into_dimensionality().unwrap();
    ...
    fn test_minimum() {
        let a: Array2<i64> = array![[1, 5, 1]];
        let b = array![[3], [-1]];
        let max = minimum(a.view(), b.view()).unwrap().into_dimensionality().unwrap();
